...
public interface StreamCreator
{
    ...
    throws Exception
    {
        CompletableFuture<HttpResponse<JsonNode>> future  = NetworkExecutor.getInstance().execute((x) -> {
            return streamV3ApiRetailer.listStreams(streamId, null);
    ...
    default void createStream(
            String streamId, int numPartitions, StreamV3Api streamV3ApiRetailer, Logger logger, Map<String, Object> query)
    throws Exception
    {
        StreamV3Api.ObjectType objectType = (StreamV3Api.ObjectType) query.get("queryType");

        CompletableFuture<HttpResponse<JsonNode>> future = NetworkExecutor.getInstance().execute((x) -> {
            return streamV3ApiRetailer.createStream(
                    streamId, "streamDescription", numPartitions, objectType, query);
